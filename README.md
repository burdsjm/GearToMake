...
The script then iterates over each item in the list. For each item, it calls the check_and_report_item function with the item name and message format. After checking each item, it waits for 2 seconds before checking the next item. The last item does not have a delay.
